    ...
    internal sealed class Processor
    {
        ...
        public async Task InitializeAsync()
        {
            try
            {
                ...
                {
                    _outputBuffer = App.Converters.GetBytes($"{Protocol.Lexicon.Readable.Error.GENERIC}{string.Format(App.Constants.ExceptionText.BUFFER_SIZE_OUT_OF_BOUNDS, GlobalVariables.MaxBufferSize)}");
            ...
            {
                _outputBuffer = App.Converters.GetBytes(string.Format(App.Constants.ExceptionText.CANCELLATION_REQUESTED, DateTime.UtcNow));
            }
            catch (Exception ex)
            {
